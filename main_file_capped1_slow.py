...
#@jit('Tuple((f8[:,:], f8[:,:],f8, f8[:,:]))(i8,i8,f8)')
...
#@jit(nopython=True)
#@jit('Tuple((f8[:,:],f8[:,:],f8[:,:]))(i8,i8,f8,i8,i8,f8,b1)',parallel=True)
def GIST_Algo(N, K, density, Sample, MaxIter_g, theta, debug):
    ...
    for s in range(Sample):
        ...
        error_g[s,0] = np.linalg.norm(x_g - x0) / np.linalg.norm(x0.toarray())
        if debug:
        ...
        for t in range(MaxIter_g-1):
            ...
            error_g[s,t+1] = np.linalg.norm(x_g - x0) / np.linalg.norm(x0.toarray())
            if debug:
...
makePlots(np.linspace(0,np.sum(t),400),v,scale="logy",grid=True)
